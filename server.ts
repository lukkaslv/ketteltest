  ...
  app.post("/api/analyze", async (req, res) => {
    try {
      ...
      const testTypeMap: Record<string, string> = {
        'bigfive': 'Большая Пятерка (Big Five)',
        'cattell': '16 Факторов Кеттела',
        'cattell_ka': 'Тест Кеттела (на грузинском)',
        'attachment': 'Стиль привязанности (ECR)',
        'attachment_ka': 'Стиль привязанности (ECR, на грузинском)'
      };
      
      const testName = testTypeMap[testType] || testType;

      const prompt = `Ты опытный психолог. Проанализируй результаты теста "${testName}".
...
${langPrompt} Оформи текст красиво, с использованием markdown (заголовки, списки).`;
  ...
  app.post("/api/compatibility", async (req, res) => {
    try {
      ...
      const prompt = `Ты семейный и корпоративный психолог-консультант. Проанализируй совместимость двух людей по результатам теста "${testName}".
